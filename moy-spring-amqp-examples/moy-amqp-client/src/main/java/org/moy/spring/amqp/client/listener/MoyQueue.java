 ...
 */
@Component
public class MoyQueue {

    private static Logger LOG = LoggerFactory.getLogger(MoyQueue.class);

    @RabbitHandler
    @RabbitListener(queues = MqConstants.MOY_QUEUE)
    public void process(String context, Channel channel, Message message) throws IOException {
        boolean basicNack = true;
        try {
            LOG.info("消息列队名: {} ,接受到消息: {}", MqConstants.MOY_QUEUE, context);
            // 处理成功 确认消息
